...
You are "Asha", a friendly AI voice agent from KZUNO — a voice AI platform for
...
class CallBridge {
  ...
  connectXai() {
    ...
    this.xai.on("open", () => {
      // Exotel uses PCM16 @ 8kHz; Vobiz uses PCM16 @ 16kHz for HD voice.
      const rate = this.type === "exotel" ? 8000 : 16000;
      const sessionUpdate = {
        audio: {
          input: {
    ...
    if (this.xai?.readyState === WebSocket.OPEN) {
      ...
      if (this.type === "vobiz") {
        try {
          ...
          console.error("[bridge] Failed to swap byte order for Vobiz audio:", e);
        }
      }
      this.xai.send(JSON.stringify({ type: "input_audio_buffer.append", audio: audioB64 }));
    }
  }
  ...
  // Re-chunk xAI audio into Exotel/Vobiz-friendly frames (multiples of 320/640 bytes).
  pushAudioToExotel(buf) {
    const minChunk = this.type === "exotel" ? 320 : 640;
    const idealChunk = this.type === "exotel" ? 3200 : 6400;
