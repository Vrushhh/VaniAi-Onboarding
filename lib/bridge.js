...
class CallBridge {
  ...
  connectXai() {
    ...
    this.xai.on("open", () => {
      // Both Exotel and Vobiz will use PCM16 @ 8kHz (standard telephony).
      const rate = 8000;
      const sessionUpdate = {
        audio: {
          input: {
    ...
    if (this.xai?.readyState === WebSocket.OPEN) {
      ...
      if (this.type === "vobiz") {
        try {
          ...
          // swap16 throws RangeError if buffer size is not a multiple of 2.
          // In case Vobiz sends an odd number of bytes, we slice to make it even.
          if (buf.length % 2 !== 0) {
