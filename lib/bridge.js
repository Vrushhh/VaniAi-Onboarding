...
class CallBridge {
  ...
  connectXai() {
    ...
    this.xai.on("open", () => {
      // Exotel uses PCM16 @ 8kHz; Vobiz uses PCMU @ 8kHz.
      const codec = this.type === "exotel" ? "audio/pcm" : "audio/pcmu";
      const sessionUpdate = {
        audio: {
          input: {
            format: { type: codec, rate: 8000 },
            transcription: { language_hint: "hi" },
          },
          output: { format: { type: codec, rate: 8000 } },
        },
        turn_detection: {
          type: "server_vad",
  ...
  // Re-chunk xAI audio into Exotel/Vobiz-friendly frames (multiples of 320/160 bytes).
  pushAudioToExotel(buf) {
    const minChunk = this.type === "exotel" ? 320 : 160;
    const idealChunk = this.type === "exotel" ? 3200 : 1600;
