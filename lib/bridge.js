  ...
  pushAudioToCarrier(buf) {
    ...
    if (!this.pacingInterval) {
      ...
      this.pacingInterval = setInterval(() => {
        ...
        const now = Date.now();
        const elapsed = Math.max(1, now - lastTickTime);
        lastTickTime = now;

        // Dynamic time-synced pacing: send exact number of chunks needed for elapsed time
        // to eliminate event-loop jitter and audio breaks/stutters on phone lines
        let chunksToSend = Math.round(elapsed / 20);
        if (chunksToSend < 1 && this.outBuffer.length >= chunkSize) chunksToSend = 1;

        while (chunksToSend > 0 && this.outBuffer.length >= chunkSize) {
        ...
        if (this.outBuffer.length < chunkSize) {
          this.silenceTicks++;
          // Stop pacing after 25 consecutive silent ticks (500ms of true idle)
          if (this.silenceTicks >= 25) {
            if (this.outBuffer.length > 0) {
              const padLength = chunkSize - (this.outBuffer.length % chunkSize);
              const padded = padLength < chunkSize ? Buffer.concat([this.outBuffer, Buffer.alloc(padLength)]) : this.outBuffer;
              for (let i = 0; i < padded.length; i += chunkSize) {
                this.sendChunk(padded.subarray(i, i + chunkSize));
              }
              this.outBuffer = Buffer.alloc(0);
            }
            this.stopPacing();
