...
class CallBridge {
  ...
  // Re-chunk and pace audio into Exotel/Vobiz-friendly frames (multiples of 320 bytes).
  pushAudioToExotel(buf) {
    this.outBuffer = Buffer.concat([this.outBuffer, buf]);

    // Start pacing interval if it is not already running
    if (!this.pacingInterval) {
      const chunkSize = 1600; // 100ms chunk at 8kHz PCM16 (16000 bytes/sec)
      ...
      this.pacingInterval = setInterval(() => {
        ...
        if (this.outBuffer.length >= chunkSize) {
          ...
          // If we run out of data, send any remaining small remainder and stop pacing
          if (this.outBuffer.length > 0) {
            this.sendChunk(this.outBuffer);
            this.outBuffer = Buffer.alloc(0);
          }
          this.stopPacing();
        }
