...
export class SarvamSessionAdapter {
  constructor(bridge) {
    ...
    // ── Context memory: persist known facts across turns so LLM never re-asks ──
    this.knownFacts = {
      brandName: null,
  ...
  onSarvamMessage(raw) {
    ...
    if (msg.type === "data" && msg.data?.transcript) {
      ...
      if (text.length > this.currentTranscript.length) {
        ...
        }, 400); // 400ms silence timeout — slightly longer to avoid premature firing
      }
    }
  }
  ...
  extractKnownFacts(text) {
    // Simple heuristic extraction — looks for brand name and product category signals
    const lowerText = text.toLowerCase();

    // Brand name: "my brand is X", "brand name is X", "I sell under X", "हमारी ब्रांड X है"
    const brandMatch = text.match(/(?:my brand(?:\s+name)?\s+is|brand(?:\s+name)?\s+is|हमारी\s+ब्रांड\s+है|हमारा\s+ब्रांड|brand\s+called|brand\s+named)\s+([A-Z][a-zA-Z0-9\s]{1,30})/i);
    if (brandMatch && !this.knownFacts.brandName) {
      this.knownFacts.brandName = brandMatch[1].trim();
      logToFile(`[Context] Captured brand name: "${this.knownFacts.brandName}"`);
    }

    // Product category: "sell X", "we sell X", "I sell X", "selling X"
    const categoryMatch = text.match(/(?:we\s+sell|i\s+sell|selling|हम\s+बेचते|बेचती\s+हूँ|बेचता\s+हूँ|विकतो|விற்கிறோம்)\s+([a-zA-Z\u0900-\u0D7F][a-zA-Z\u0900-\u0D7F\s]{1,40})/i);
    if (categoryMatch && !this.knownFacts.productCategory) {
      this.knownFacts.productCategory = categoryMatch[1].trim();
      logToFile(`[Context] Captured product category: "${this.knownFacts.productCategory}"`);
    }

    // Pain point: RTO, returns, carts, support
    if (!this.knownFacts.painPoint) {
      if (/\bRTO\b|return\s+to\s+origin|cod\s+fail/i.test(text)) this.knownFacts.painPoint = "high RTO";
      else if (/abandon(?:ed)?\s+cart|cart\s+recovery/i.test(text)) this.knownFacts.painPoint = "abandoned carts";
      else if (/support\s+call|missed\s+call|customer\s+care/i.test(text)) this.knownFacts.painPoint = "missed customer support calls";
    }
  }
  ...
  async triggerResponse(text) {
    ...
    // Extract known facts before pushing to history
    this.extractKnownFacts(text);
    this.history.push({ role: "user", content: text });

    // ── Language detection with persistence ──
    // Use ASR-reported language if available AND utterance is multi-word (>= 2 words)
    // This prevents single-word utterances like "haan" or "okay" from flipping the language
    let detectedLang = detectTtsLanguage(text);
    const wordCount = text.trim().split(/\s+/).length;

    if (this.currentSttLangCode && this.currentSttLangCode !== "unknown") {
      if (wordCount >= 2) {
        // Multi-word utterance — trust ASR, update confirmed language
        detectedLang = this.currentSttLangCode;
        this.confirmedLang = detectedLang;
        logToFile(`Using ASR-reported language: ${detectedLang} (confirmed, ${wordCount} words)`);
      } else if (this.confirmedLang) {
        // Single-word utterance — preserve confirmed language to avoid flipping
        detectedLang = this.confirmedLang;
        logToFile(`Single-word utterance — keeping confirmed language: ${detectedLang}`);
      } else {
        detectedLang = this.currentSttLangCode;
        this.confirmedLang = detectedLang;
      }
    } else if (this.confirmedLang && wordCount < 3 && detectedLang === "en-IN") {
      // Heuristic fallback: if text heuristic returns English on a short utterance,
      // keep confirmed language to avoid language flips on fillers ("ok", "yes", "fine")
      detectedLang = this.confirmedLang;
      logToFile(`Short utterance, keeping confirmed language: ${detectedLang}`);
    } else if (wordCount >= 2) {
      this.confirmedLang = detectedLang;
    }

    // Lock the language for ALL TTS sentences in this turn to prevent language switching mid-response
