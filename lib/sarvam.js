...
export class SarvamSessionAdapter {
  ...
  sendCallerAudio(b64) {
    if (this.ws?.readyState === WebSocket.OPEN) {
      try {
        ...
        console.error("[bridge-sarvam] Failed to process caller audio:", err);
      }
    }
  }
  ...
  sendAccumulatedAudio() {
    if (this.ws?.readyState === WebSocket.OPEN && this.accumulatedPcm8.length > 0) {
      try {
        ...
        console.error("[bridge-sarvam] Failed to send accumulated audio:", err);
      }
    }
  }
  ...
  onSarvamMessage(raw) {
    ...
    if (msg.type === "data" && msg.data?.transcript) {
      ...
      if (text !== this.currentTranscript) {
        console.log(`[sarvam-stt] Transcript updated: "${text}"`);
        this.currentTranscript = text;

        // Reset the silence timeout ONLY when new speech content is transcribed
        if (this.silenceTimeout) clearTimeout(this.silenceTimeout);
        this.silenceTimeout = setTimeout(() => {
          console.log(`[sarvam-stt] Silence detected after speech. Triggering response...`);
  ...
  async speakInitialGreeting() {
    ...
    console.log("[bridge-sarvam] Play initial greeting (disclosure line only)...");
