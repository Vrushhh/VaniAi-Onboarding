...
function logToFile(msg) {
  try {
    ...
    fs.appendFileSync("/Users/vrushabhgamare/Documents/VoiceAI_Onboarding/server-debug.log", formatted);
  } catch (err) {
...
export class SarvamSessionAdapter {
  ...
  connect() {
    ...
    this.ws.on("close", (code, reason) => {
      ...
      this.bridge.teardown("sarvam closed");
    });
    this.ws.on("error", (e) => {
      logToFile(`Sarvam STT WS error: ${e.message}`);
      this.bridge.teardown(`sarvam error: ${e.message}`);
    });
  }
    ...
    if (this.ws?.readyState === WebSocket.OPEN) {
      try {
        ...
        if (Date.now() >= this.speakingEndTime) {
          ...
          logToFile("Caller audio ignored: echo suppression active (agent is speaking)");
        }
      } catch (err) {
  ...
  sendAccumulatedAudio() {
    if (this.ws?.readyState === WebSocket.OPEN && this.accumulatedPcm8.length > 0) {
      try {
        ...
        logToFile(`Sending accumulated audio packet to STT. Size: ${this.accumulatedPcm8.length} bytes.`);
        this.ws.send(
          JSON.stringify({
            audio: {
  ...
  onSarvamMessage(raw) {
    ...
    if (msg.type === "data" && msg.data?.transcript) {
      ...
      // Check if the transcript has actually changed/grown
      if (text !== this.currentTranscript) {
        logToFile(`Transcript updated: "${text}"`);
        this.currentTranscript = text;

        // Reset the silence timeout ONLY when new speech content is transcribed
