...
- Conversational Warmth: Use natural fillers & exclamations matched strictly to the turn's language:
  - For Hindi turns: "जी बिल्कुल!", "बहुत बढ़िया!", "समझ गई!", "अच्छा!", "सही बात है!" (NEVER use Marathi words like "नक्कीच" in Hindi!).
  - For Marathi turns: "हो नक्कीच!", "छान!", "समजलं!", "बरोबर!"
  ...
  - For English/Hinglish turns: "Makes total sense!", "Oh, wonderful!", "Understood!", "Ah, interesting!", "Right!", "Definitely!"
...
export class SarvamSessionAdapter {
  ...
  onSarvamMessage(raw) {
    ...
    if (msg.type === "data" && msg.data?.transcript) {
      ...
      // ── Instant Barge-in / Interruption: Agent takes a step back ──
      // If user speaks while agent is speaking or generating, immediately cancel audio and listen
      if (Date.now() < this.speakingEndTime || this.processingTurn) {
        const isRealSpeech = isSpeakable(text) && (text.length >= 3 || text.includes(" ") || ["haan", "no", "yes", "stop", "wait", "hold"].includes(text.toLowerCase()));
        if (isRealSpeech) {
      ...
      if (text.length > this.currentTranscript.length) {
        ...
        this.silenceTimeout = setTimeout(() => {
          ...
          logToFile(`Silence detected. Triggering fast response for: "${finalText}"`);
          this.lastTriggeredTranscript = finalText;
          this.triggerResponse(finalText);
        }, 300); // 300ms silence timeout — fast 300ms turn-taking reply speed
      }
    }
  }
  ...
  async triggerResponse(text) {
    // ── Robust Language Detection: Protect English Conversation ──
    // Never switch out of English unless user explicitly speaks in native Indic script or clear Indic keywords
    ...
    if (/[\u0900-\u097F]/.test(text)) {
      ...
      detectedLang = "hi-IN";
    } else if (this.confirmedLang && this.confirmedLang !== "en-IN" && text.trim().split(/\s+/).length < 3) {
