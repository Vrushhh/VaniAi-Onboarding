...
export class SarvamSessionAdapter {
  ...
  extractKnownFacts(text) {
    ...
    const brandMatch = text.match(/(?:my brand(?:\s+name)?\s+is|brand(?:\s+name)?\s+is|we\s+are|i\s+run|store(?:\s+name)?\s+is|हमारी\s+ब्रांड\s+है|हमारा\s+ब्रांड|brand\s+called|brand\s+named)\s+([a-zA-Z0-9\s]{2,30})(?:\s+and|\s+we|\s+in|\.|\,|$)/i);
    if (brandMatch && brandMatch[1] && !this.knownFacts.brandName) {
      let candidate = brandMatch[1].replace(/\b(?:and|we|in|for|sell|a|the)\b.*$/i, "").trim();
    ...
    const categoryKeywords = ["skincare", "beauty", "clothing", "apparel", "fashion", "footwear", "shoes", "electronics", "jewellery", "jewelry", "food", "beverages", "supplements", "fitness", "home decor"];
    for (const cat of categoryKeywords) {
      if (lowerText.includes(cat) && !this.knownFacts.productCategory) {
        this.knownFacts.productCategory = cat;
        logToFile(`[Context] Captured product category: "${cat}"`);
        break;
      }
    }
    ...
    if (!this.knownFacts.painPoint) {
      if (/\bRTO\b|return\s+to\s+origin|cod\s+fail|return/i.test(text)) this.knownFacts.painPoint = "high RTO on COD orders";
  ...
  async fetchTts(text, lang = "hi-IN") {
    ...
    // Phonetic replacement: convert KZUNO/kzuno to "Kizuno" (or Devanagari "कहीज़ूनो")
    // so Bulbul pronounces it fluently in one go without spelling letter by letter "K-eye-zuno"
    let cleanText = text;
    if (/hi|mr|bn|gu|ta|te|kn|ml|pa|od/.test(lang)) {
      cleanText = cleanText.replace(/\b(?:KZUNO|Kzuno|KiZUNO|kzuno)'?s?\b/gi, "कहीज़ूनो");
    } else {
      cleanText = cleanText.replace(/\b(?:KZUNO|Kzuno|KiZUNO|kzuno)'?s?\b/gi, "Kizuno");
    }
