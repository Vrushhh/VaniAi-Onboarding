...
export class SarvamSessionAdapter {
  ...
  connect() {
    console.log("[bridge-sarvam] Connecting to Sarvam STT WebSocket...");
    this.ws = new WebSocket("wss://api.sarvam.ai/speech-to-text/ws", {
      headers: {
    ...
    this.ws.on("open", () => {
      console.log("[bridge-sarvam] Sarvam STT WebSocket opened. Sending config...");
      // Send initial configuration for Saarika (STT)
      this.ws.send(
        JSON.stringify({
          config: {
            model: "saaras:v3",
  ...
  downsample24to8(buffer) {
    // Strip 44-byte WAV header to get raw 16-bit PCM samples
    const raw = buffer.slice(44);
