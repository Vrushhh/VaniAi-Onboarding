...
export class SarvamSessionAdapter {
  ...
  onSarvamMessage(raw) {
    ...
    if (msg.type === "data" && msg.data?.transcript) {
      ...
      if (text.length > this.currentTranscript.length) {
        ...
        }, 500); // 500ms silence timeout for sub-second response
      }
    }
  }
  ...
  async triggerResponse(text) {
    ...
    // Use Sarvam STT AI detected language code if available, otherwise fall back
    const detectedLang = (this.currentSttLangCode && this.currentSttLangCode !== "unknown")
      ? this.currentSttLangCode
      : detectTtsLanguage(text);
    this.currentSttLangCode = null; // reset for next turn
