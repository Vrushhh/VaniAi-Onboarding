...
You are "Vaani", a friendly AI voice agent from KZUNO — a voice AI platform for Indian D2C brands.
You are on a short OUTBOUND DEMO CALL that the user requested on the KZUNO website.

CRITICAL RULES:
- EVERY reply must be 1-3 short Hindi sentences MAXIMUM. Never exceed 3 sentences.
- This is a PHONE CALL. Speak naturally and briefly like a real conversation.
- NEVER do a role-play, dialogue, script, or act out characters.
- NEVER use quotes, parentheses, stage directions, or English sentences.
- NEVER write "Namaste!", "Thank you", "Goodbye" in English. Always use Hindi: "नमस्ते", "धन्यवाद", "अलविदा".
- Respond ONLY in Hindi or Hinglish. No pure English.

Flow:
1. AI disclosure line has already been played. Greet warmly and confirm this is the demo call.
2. Briefly explain what KZUNO agents do: order confirmation, lead qualification, payment reminders, 24x7 support in Indian languages.
3. If they ask for a demo, describe how it works — do NOT act it out or simulate dialogue.
4. Close by inviting them to sign up, thank them, say goodbye.

- Never claim to be human.
- If the listener says "stop" or "band karo" — say goodbye immediately in one sentence.
`.trim();
...
export class SarvamSessionAdapter {
  ...
  onSarvamMessage(raw) {
    ...
    if (msg.type === "data" && msg.data?.transcript) {
      ...
      // Ignore STT inputs while a turn is already processing (LLM request in-flight)
      if (this.processingTurn) {
        return;
      }

      // If the agent is currently speaking and we detect real user speech,
      // treat it as a barge-in: cancel queued audio and prepare for new turn
      if (Date.now() < this.speakingEndTime) {
        // Only treat as barge-in if the transcript looks like real user speech (>2 chars)
        // and is genuinely new content (not echo of agent's own speech)
        if (text.length > 2 && text.length > this.currentTranscript.length) {
          logToFile(`Barge-in detected during agent speech: "${text}". Cancelling playback.`);
          this.bridge.onSpeechStarted(); // Clear phone audio buffer
          this.ttsQueue = Promise.resolve(); // Reset TTS queue
          this.currentTurnToken = {}; // Invalidate old TTS fetches
          this.speakingEndTime = 0; // Stop echo suppression
        } else {
  ...
  async speakInitialGreeting() {
    ...
    if (cachedDisclosureB64) {
      ...
      this.ttsQueue = this.ttsQueue.then(async () => {
        ...
        const durationMs = 5488; // 87808 bytes / 16000 bytes/sec * 1000
  ...
  async triggerResponse(text) {
    ...
    try {
      logToFile("Calling Sarvam Chat Completions REST API...");
      ...
      if (reply) {
        ...
        // Clean the reply: strip parenthetical stage directions, stray quotes, and
        // filter out sentences that contain no Hindi/Devanagari characters.
        const cleaned = reply
          .replace(/\(.*?\)/g, "")       // Remove (stage directions)
          .replace(/["""'']/g, "")        // Remove all quote characters
          .replace(/^\s*\n/gm, "");       // Remove blank lines
        
        // Split reply into sentences to fetch TTS concurrently
        const sentences = splitIntoSentences(cleaned).filter(s => {
          // Must have at least one Devanagari character to be speakable
          return /[\u0900-\u097F]/.test(s) && s.length > 1;
        });
        
        // Limit to 4 sentences max to prevent 30-second monologues
