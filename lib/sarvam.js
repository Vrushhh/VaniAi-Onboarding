...
const MARATHI_MARKERS = /(?:आहे|नाही|करू|शकत|तुम्ही|तुम्हाला|मला|काय|मध्ये|होतो|होती|होते|आम्ही|आम्हाला|तुमच|तुमची|तुमचे|आमच|आमची|आमचे|करतो|करते|करतात|विकतो|विकते|विकतात|येतो|येते|येतात|दिवसात|दिवसाला|करायचं|करायचे|करणार|आपण|सांगा|बोला|पाहिजे|पाहिजेल|छान|काही|कसं|कशी|कसा|कसे|ऐका|नका|चालेल|होणार|केलं)/i;
...
function detectTtsLanguage(text) {
  ...
  // Other Indic scripts (Bengali, Tamil, Telugu, Kannada, Malayalam, Gujarati, Punjabi, Odia)
  for (const [lang, regex] of Object.entries(LANG_MAP)) {
    if (lang === "hi-IN" || lang === "mr-IN") continue;
    if (regex.test(text)) return lang;
  }

  // Transliterated Latin script checks
...
## MANDATORY MULTILINGUAL RULE (CRITICAL)
- YOU MUST REPLY IN THE EXACT SAME LANGUAGE SPOKEN BY THE CALLER ON THAT TURN.
- If the caller speaks Hindi / Hinglish (e.g. "5-10 order आता है", "क्या कर सकते हो", "ब्रांड ओनर हूँ"), YOU MUST REPLY ENTIRELY IN HINDI (in Devanagari script).
- If the caller speaks Marathi (e.g. "ऐका ना", "काय काय विकतात", "तुमची काय ऑफर आहे"), YOU MUST REPLY ENTIRELY IN MARATHI (in Devanagari script).
- If the caller speaks Gujarati, Bengali, Tamil, Telugu, Kannada, Malayalam, Punjabi, or Odia, YOU MUST REPLY ENTIRELY IN THAT SAME REGIONAL LANGUAGE.
- NEVER reply in English when the caller speaks in Hindi, Marathi, or any regional Indian language!

## CONVERSATION FLOW (ADAPT TO CALLER'S LANGUAGE)
1. GREETING:
   - Initial greeting is played. When caller confirms they own a D2C or Shopify brand, acknowledge warmly in their language and ask for their brand name & product category.
2. DISCOVERY & QUALIFICATION:
   - Listen to their product category. Acknowledge their specific product category (e.g. textiles, furniture, apparel) warmly in their language.
   - Ask how many daily orders or customer inquiries they handle in their language.
3. PITCHING VALUE:
   - Pitch KZUNO's voice AI features (address confirmation, COD RTO reduction by 25%, abandoned cart recovery, 24x7 support in Indian languages) tailored to their business in their language.
4. CALL TO ACTION:
   - Invite them to register a free account at console dot kzuno dot in in their language.

## STYLE & GUARDRAILS
- Keep responses extremely short (1-2 sentences max per turn). Telephony conversations require rapid back-and-forth; do not monologue.
- Listen carefully to what the caller says and answer their specific points. Never ignore their input or repeat static questions.
- Never use markdown formatting (asterisks, hashes, bullet points).
- If asked technical questions, explain that KZUNO connects with Shopify, WooCommerce, and shipping gateways via APIs.
- If asked about pricing, mention our free starter tier and custom plans starting at 2 rupees per call.
`.trim();
...
export class SarvamSessionAdapter {
  ...
  onSarvamMessage(raw) {
    ...
    if (msg.type === "data" && msg.data?.transcript) {
      ...
      if (Date.now() < this.speakingEndTime) {
        if (text.length > 1 && text.length > this.currentTranscript.length) {
      ...
      if (text.length > this.currentTranscript.length) {
        ...
        }, 1100); // 1100ms allows STT transcript to finalize full sentence before LLM call
      }
    }
  }
  ...
  async triggerResponse(text) {
    ...
    let langInstructionText = "The caller just spoke in ENGLISH. Respond naturally in English/Hinglish.";
    if (detectedLang === "hi-IN") {
      langInstructionText = "CRITICAL MANDATE: The caller just spoke in HINDI. YOU MUST RESPOND ENTIRELY IN HINDI (using Devanagari script). DO NOT USE ENGLISH!";
    } else if (detectedLang === "mr-IN") {
      langInstructionText = "CRITICAL MANDATE: The caller just spoke in MARATHI. YOU MUST RESPOND ENTIRELY IN MARATHI (using Devanagari script). DO NOT USE ENGLISH!";
    } else if (detectedLang === "gu-IN") {
      langInstructionText = "CRITICAL MANDATE: The caller just spoke in GUJARATI. YOU MUST RESPOND ENTIRELY IN GUJARATI. DO NOT USE ENGLISH!";
    } else if (detectedLang === "ta-IN") {
      langInstructionText = "CRITICAL MANDATE: The caller just spoke in TAMIL. YOU MUST RESPOND ENTIRELY IN TAMIL. DO NOT USE ENGLISH!";
    } else if (detectedLang === "te-IN") {
      langInstructionText = "CRITICAL MANDATE: The caller just spoke in TELUGU. YOU MUST RESPOND ENTIRELY IN TELUGU. DO NOT USE ENGLISH!";
    } else if (detectedLang === "bn-IN") {
      langInstructionText = "CRITICAL MANDATE: The caller just spoke in BENGALI. YOU MUST RESPOND ENTIRELY IN BENGALI. DO NOT USE ENGLISH!";
    }
  ...
  downsample24to8(buffer) {
    ...
    for (let i = 0; i < outSamples * 3; i += 3) {
      ...
      // 1.2x gain boost for maximum voice clarity over phone lines
      const boosted = Math.round(weighted * 1.2);
      const clamped = Math.max(-32768, Math.min(32767, boosted));
