...
export class SarvamSessionAdapter {
  ...
  onSarvamMessage(raw) {
    ...
    if (msg.type === "data" && msg.data?.transcript) {
      ...
      // Barge-in detection during agent speech (only trigger on speakable user speech)
      if (Date.now() < this.speakingEndTime) {
        if (isSpeakable(text)) {
          ...
          return; // Ignore noise/echo artifacts while agent is speaking
        }
      }
  ...
  async triggerResponse(text) {
    ...
    // Also honour the ASR language_code if Sarvam detected a language for this utterance
    let detectedLang = detectTtsLanguage(text);
    if (this.currentSttLangCode && this.currentSttLangCode !== "unknown") {
      // Map ASR lang code → TTS lang code (they use the same xx-IN format)
      detectedLang = this.currentSttLangCode;
      logToFile(`Using ASR-reported language: ${detectedLang} (overriding text heuristic)`);
    }
    ...
    try {
          ...
          try {
            ...
            if (sentenceEndMatch && sentenceCount < MAX_SENTENCES) {
              ...
              if (isSpeakable(cleaned)) {
                // Always use the turn-locked language — never re-detect per sentence chunk
                // This prevents mid-response language switches (e.g. Tamil LLM reply triggering hi-IN TTS)
      ...
      if (sentenceBuffer.trim() && sentenceCount < MAX_SENTENCES && this.currentTurnToken === turnToken) {
        ...
        if (isSpeakable(cleaned)) {
          // Always use the turn-locked language for remainder chunks too
          const lang = this.currentTurnLang;
          const subSentences = breakLongSentence(cleaned, 70);
          for (const sub of subSentences) {
            if (sentenceCount >= MAX_SENTENCES) break;
      ...
      // Reset ASR lang so next turn picks fresh detection
  ...
  downsample24to8(buffer) {
    ...
    let raw = dataOffset !== -1 ? buffer.slice(dataOffset + 8) : buffer.slice(44);
